...
import { getSalonStats } from "@/services/admin-analytics";
import { formatKopecks } from "@/domain/pricing/money";
import { salonDayKey, shiftDayKey } from "@/domain/scheduling/salon-time";
import { RevenueChart } from "@/components/revenue-chart";

/*
  Аналитика салона целиком — почти копия /master/stats, но без фильтра
  по мастеру и с добавленной разбивкой «кто сколько принёс». Домен тот
  же (summarizePeriod принимает любой набор записей), сервис другой
  (getSalonStats вместо getMasterStats).
*/
...
export default async function AdminStatsPage({
  ...
  const stats = await getSalonStats(start, end);
  const { summary } = stats;

  const presetLink = (days: number) =>
    `/admin/stats?from=${shiftDayKey(today, -(days - 1))}&to=${today}`;
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <h1 className="mb-1 text-xl">Аналитика салона</h1>
        ...
        <div className="grid content-start gap-6">
          <div className="stat">
            ...
            <span className="text-muted text-xs">
              ...
              по графику всех активных мастеров
            </span>
          </div>
      ...
      <h2 className="mb-2 text-lg">По мастерам</h2>
      {stats.byMaster.length > 0 ? (
        <ul className="mb-10">
          {stats.byMaster.map((master) => (
            <li
              key={master.masterId}
              className="border-sand flex flex-wrap items-baseline gap-x-4 border-b py-3"
            >
              <span>{master.displayName}</span>
              <span className="text-muted text-sm">
                {master.completed} из {master.total} · загрузка{" "}
                {percent(master.loadShare)}
              </span>
              <span className="font-display ml-auto text-sm tabular-nums">
                {formatKopecks(master.revenueKopecks)}
              </span>
            </li>
          ))}
        </ul>
      ) : (
        <p className="text-muted mb-10 text-sm">Активных мастеров нет.</p>
      )}
        ...
        <ul>
          {summary.topServices.map((service) => (
            ...
            >
              ...
              <span className="text-muted text-sm">{service.count} раз</span>
              <span className="font-display ml-auto text-sm tabular-nums">
